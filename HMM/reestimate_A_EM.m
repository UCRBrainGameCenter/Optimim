...
P_list = cell(K,1);
       for k=1:K
            ...
            q_1_old = exp(logP1*10^-5)*(pi_1);%To make sure that the values don't fall below 0, multiply with 2*10^-10
            q_2_old = exp(logP2*10^-5)*(pi_2);
            q_3_old = exp(logP3*10^-5)*(pi_3);
             q_4_old = exp(logP4*10^-5)*(pi_4);
